...
describe("ProfileNotificatinos", () => {
  ...
  beforeAll(async () => {
    ...
    if (!result) {
      throw new Error(
        "Error while booting profileNotifications -> beforeAll: Server could not be started"
      );
    }
  ...
  it("create profileNotification", async () => {
    const response = await fetch(TARGET_URL + "/profileNotifications/", {
      ...
      body: JSON.stringify({
        ...
        notificationId: faker.datatype.number(100),
        ack: faker.random.arrayElement(['deliver','undeliver'])
      })
    });
  ...
  it("get single profileNotification", async () => {
    const profileId = profileNotifications[profileNotifications.length - 1].id;
    const response = await fetch(`${TARGET_URL}/profileNotifications/${profileId}`);
    const data = await response.json();
    expect(response.status).toBe(200);
  ...
  it("patch profileNotification", async () => {
    const profileId = profileNotifications[profileNotifications.length - 1].id;
    const response = await fetch(`${TARGET_URL}/profileNotifications/${profileId}`, {
      method: "PATCH",
      headers,
  ...
  it("delete profileNotification", async () => {
    const profileId = profileNotifications[profileNotifications.length - 1].id;
    const response = await fetch(`${TARGET_URL}/profileNotifications/${profileId}`, {
      method: "DELETE",
      headers
